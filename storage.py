...
class NovelStorage:
    ...
    async def get_active_novel(self, session_id: str) -> Optional[Novel]:
        ...
        if novel_id is None:
            return None
        return self.load_novel(novel_id)
    ...
    def save_novel(self, novel: Novel):
        novel.updated_at = __import__("datetime").datetime.now().isoformat()
        path = self._novel_path(novel.id)
        path.write_text(
            json.dumps(novel.to_dict(), ensure_ascii=False, indent=2), encoding="utf-8"
        )

    def load_novel(self, novel_id: str) -> Optional[Novel]:
    ...
    def list_novel_summaries(self) -> list[dict]:
